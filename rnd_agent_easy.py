...
class RNDAgentEasy(agent.AttributeSavingMixin, agent.BatchAgent):
    """Deep Q-Network + Exploration by Random Network Distillation.
    ...
    def __init__(self, q_function, rnd, optimizer, opt_rnd, replay_buffer, gamma,
        ...
        #self.obs_norm = UpdateMeanStd(shape=(2))
        self.obs_norm = UpdateMeanStd(shape=(4))
    ...
    def phi_rnd(self, x):
        return np.asarray(np.clip((x - self.obs_norm.mean)/np.sqrt(self.obs_norm.var), -1.0, 1.0), dtype=np.float32)
